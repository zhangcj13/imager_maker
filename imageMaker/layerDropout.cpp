...
namespace imgmaker
{
	...
	layerDropout::layerDropout(const float _rate)
	{
		setParamaters(_rate);
	}
	layerDropout::~layerDropout()
	...
	DEFINE_LAYER_TYPE(layerDropout, "layerDropout");
	std::string layerDropout::getLayerType() const
	{
		return layerType;
	}
	void layerDropout::setParamaters(const float _rate)
	{
		rate = _rate;
	}
	std::string layerDropout::serializeToString() const
	{
	...
	void layerDropout::serializeFromString(const std::string content)
	{
	...
	void layerDropout::solveInnerParams()
	{
		if (!mask.get())
		{
	...
	void layerDropout::forward(const std::shared_ptr<DataBucket> prev, std::shared_ptr<DataBucket> next)
	{
	...
	void layerDropout::backward(std::shared_ptr<DataBucket> prev, const std::shared_ptr<DataBucket> next,
